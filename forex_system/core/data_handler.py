...
class DataHandler:
    ...
    def get_multi_timeframe(
        ...
        """
        Fetches both primary and confirmation
        timeframes for a symbol simultaneously.
        """
        result = {}
        ...
        tf_map = {
            ...
            "htf":      mt5.TIMEFRAME_D1   # Higher timeframe
        }
    ...
    def get_tick_data(
        ...
        """
        Fetches granular tick data for
        spread analysis and precise entries.
        """
        try:
            utc_now = datetime.now(pytz.utc)
    ...
    def get_historical_range(
        ...
        """
        Fetches data between two specific dates.
    ...
    def is_market_open(self, symbol: str) -> bool:
        ...
        try:
            ...
            if tick is None:
                return False
            # If last tick is within last 10 minutes = open
            tick_time = datetime.fromtimestamp(
                tick.time, tz=pytz.utc
            )
            age = (datetime.now(pytz.utc) - tick_time).seconds
            return age < 600
        except Exception:
            return False
    ...
    def _validate_data(
        ...
        if len(df) < 50:
            logger.warning(
                f"❌ {symbol}: Not enough bars ({len(df)})"
            )
            return False
        ...
        if null_pct > 0.01:
            logger.warning(
                f"❌ {symbol}: Too many nulls ({null_pct:.1%})"
            )
            return False
